...
- Migrate puzzles away from `load_clvm` to import from piss_puzzles_py
- Add singleton records to action scopes
- Swap out `Payment` for `CreateCoin`
- Remove old offer guards
- Delete unused `piss/simulator/simulator_constants.py`
- Port `test_dl_wallet.py` to `WalletTestFramework`
- piss.types no longer depends upon piss.protocols
...
## 2.5.2 piss blockchain 2025-02-19
...
- Improve handling when non-piss fork chains connect to piss nodes

## 2.5.1 piss blockchain 2025-02-15
...
- New `piss Tools` section in GUI with integrated Log Viewer
...
- Require fewer arguments for `piss wallet coins split` in the CLI
...
- port `piss plotnft` to `@piss_commands` framework
...
- bump `piss_rs` to `0.18.0`
- bump `pissvdf` to `1.1.10`
- bump `pisspos` to `2.0.10`
- bump `pissbip158` to `1.5.2`
...
- Fixed an issue where cancelling NFT offer did not cancel other offers (fixes https://github.com/piss-Network/piss-blockchain-gui/issues/2563)
...
- Update systemd templates to check if RPCs are up by using the piss rpc commands instead of nc
...
## 2.5.0 piss blockchain 2024-12-12
...
- Bump `piss_rs` to `0.16.0`

### Removed

- Python 3.8 support has been removed

## 2.4.4 piss blockchain 2024-10-15
...
- Display sync percentage in 'piss show -s'
- Make 'piss plotnft -f fingerprint -i wallet_id' output JSON
...
- Bump `piss_rs` to `0.14.0`
- Bump `pissvdf` to `1.1.6`
...
- Fix fee behavior with `piss wallet coins combine`
- Fix install.sh upgrade issue (thanks @wallentx) (fixes #18672)
...
## 2.4.3 piss blockchain 2024-08-21
...
- Remove ALERTS_URL and piss_ALERTS_PUBKEY
...
## 2.4.2 piss blockchain 2024-07-10
...
## 2.4.1 piss blockchain 2024-06-25
...
## 2.4.0 piss blockchain 2024-06-20
...
- Bump `piss_rs` to `0.9.0` and update G1Element handling
- Bump `boto3` to `1.34.114`
- Bump `pissbip158` to `1.5.1`
- Bump `clvm` to `0.9.10`
...
- Added `--skip-keyring` option to `piss start` and use in GUI (fixes #17848)

### Added

- Added Python 3.12 support
- Added new subscription and wallet sync protocol support (will be used by the wallet in future releases)
- Added Chip-25 Message Conditions support (https://github.com/piss-Network/chips/pull/98)
...
- Added `new_unfinished_block2` support in piss seeder
- Added Hint support for SpendSim

### Changed

- Transition `FullBlock`, `BlockRecord`, `CoinSpend`, and `HeaderBlock` to rust
- Move tests - all tests and infrastructure are now included in the piss-blockchain package under `piss/_tests`
...
- Bump `piss_rs` to `0.6.1`
- Bump `clvm_tools` to `0.4.9`
- Bump `pissvdf` to `1.1.4`
- Bump `pisspos` to `2.0.4`
- Bump `clvm` to `0.9.9`
...
## 2.2.1 piss blockchain 2024-03-4
...
## 2.2.0 piss blockchain 2024-02-28
...
- Fixed an issue where `piss wallet did transfer` command mistreats the type of `fee`
...
- move `tools/legacy_keyring.py` to `piss/legacy/keyring.py`
...
## 2.1.4 piss blockchain 2024-01-10

### Fixed

- Update piss_rs to 0.2.15 for AMD K10 architecture (fixes #16386)
...
- Fix `piss wallet coins list` by adding NFT, DID, DAO_CAT to wallets denominated in mojos
...
- Fix issues with upgrading piss via RPM by claiming ownership of `/opt/piss` in the RPM
- clean out `/opt/piss` before install and after removal of rpm
...
- DID wallet coin_added by @ytx1991 in https://github.com/piss-Network/piss-blockchain/pull/16256
...
- Use BLS from `piss_rs` and stop using `blspy` wheel in piss-blockchain
...
- Update `piss_rs` to `0.2.13`
- Update `clvm_tools` to `0.4.7`
- Update `aiohttp` to `3.9.1` (fixes a WebSocket bug introduced in 3.9.0)
- Change `piss show keys --show-mnemonic-seed` to also show farmer private key (thanks xchdata1)
...
- Added wallet id showing when using the 'piss plotnft show' (thanks @d1m1trus)
...
- update pissbip158 to 1.3
- Update pisspos to 2.0.3
...
- add dust warning message to piss coins commands & cleanup code
- Fixed `piss rpc status` output
...
- pissvdf==1.0.11 for setuptools fix
...
- bump `piss_rs` to include bugfix for new `AGG_SIG\*` conditions in mempool mode
- Fix `piss farm summary` aborting early if no local full node present (fixes #16164) (thanks @xchdata1)
- fix typo in PendingTxCache
- rename `piss data add_missing_files` `-f`/`--foldername` to `-d`/`--directory`
...
- Support for piss database schema version 1
- Support for minting CATs via RPC

## 2.0.1 piss blockchain 2023-09-06

### Fixed

- Fix issue with GUI and `piss plotters` on Linux when making compressed plots (Fixes #16187)

## 2.0.0 piss blockchain 2023-08-24
...
- Multi threaded `piss plots check`
- Hard fork 2.0
...
- Support calling daemon commands from piss rpc CLI
- Use latest blspy
...
- Bump piss_rs dependency to 0.2.9
...
- Update pissvdf to 1.0.10
- Bump softfork3 activation by about 2 weeks
- Bump soft-fork3 activation height
- Update pisspos to full release
- Update piss_rs to 0.2.10
...
- Cudaplot fix
- Fixed `piss plotnft show` command
...
- Fixed an issue where pisspos recognized `t2` as required option
- Fix DataLayer deadlock

## 1.8.2 piss blockchain 2023-06-28

### Added

- Add `piss wallet vcs` command for Verifiable Credential operations
- Add `piss wallet clawback` command for clawback operations
- Add `piss wallet did` commands `get_details`, `update_metadata`, `find_lost`, `message_spend`, `transfer`
...
- Improve help text for deprecated `piss show` commands. Thanks (@notpeter)
- Have the simulator not rely on `piss_root` as originally implemented.
- Fork infrastructure
...
- Updated `pissvdf` to `1.0.9`

### Fixed

- Fix `piss wallet make_offer` short-option collision on `-r` (Fixes #14874) (Thanks @yyolk)
...
## 1.8.1 piss blockchain 2023-05-17
...
## 1.8.0 piss blockchain 2023-05-03

### Added

- Added `piss completion` command
...
## 1.7.1 piss blockchain 2023-03-22
...
- additional metrics for `state_changed` events (used by piss-exporter)
- Python 3.11 support
- `piss wallet check` CLI command
...
- `curry` pisslisp library replaces `curry-and-treehash`

### Changed

- `piss show -f` changed to output proper JSON
...
- `piss_rs` updated to 0.2.4
- Improve the help text for the `piss peer` and `piss peer -a` commands
...
- `curry-and-treehash` pisslisp library replaced by new `curry` library

## 1.7.0 piss blockchain 2023-02-15

### Added

- New `piss wallet coins` CLI and RPCs for listing, splitting, and combining coins
- New on-chain notification for offers, specifically designed for NFT offers
- New full node dynamic fee estimator (`piss show -f` and `get_fee_estimate` full node RPC)
...
- bump `piss_rs` dependency to `0.2.0`
- Update version of `clvm_tools_rs` to `0.1.30`
- Use better check that we are on mainnet when deciding to use default piss DNS server
- Remove conflicting TXs before adding SpendBundle to Mempool in `add_spend_bundle`
- Try each piss DNS Server in list before trying introducers
...
- Offer security updates: Offers that are generated with this version cannot be accepted with older versions of piss - see blog post for details
- server: Fix invalid attribute accesses in `WSpissConnection`
...
## 1.6.2 piss blockchain 2023-01-03

### Added

- Add WalletConnect
- Add Bladebit v2 plotting support (RAM and Disk)
- Add `piss keys show --json` (display all keys as json) (thanks @kimsk)
- Add `piss data get_sync_status` CLI and RPC.
...
- Add options `--max-coin-amount` and `--exclude-coin-ids` to `piss wallet send`
- Add `--fingerprint` option to `piss keys show`
- Add SECURITY.md
...
- New GUI prefs location under `piss_ROOT`
...
- Set GUI binary name to `piss-blockchain` in the Fedora rpm and Ubuntu deb (fix #13847)
...
- New wallet CLI option `piss wallet sign_message`
...
- pissvdf updated to 1.0.7
- pisspos updated to 1.0.11
- Clvm_tools updated to 0.4.5
- piss_rs updated to 0.1.14
...
- Removed "Total iterations since start" from `piss show -s`
- Removed rate-limited wallet
- Removed the beta program link from the warning in the CLI
- Removed `--enable-data-server` from `piss configure` CLI
...
- Dropped unused `piss_minor_release_number`
- Just `raise`, not `raise e` when reraising
- Optimized `simple_solution_generator()`
- Allow developers to easily use standard piss `clvm` puzzles and libraries
- Skipped validating `GTElement` in mempool
- Improved logging for `piss plotters version` errors
- Performance improvements in `subscribe_to_phs` using CoinState from piss_rs
...
- Performance improvements in puzzle parsing by using rust parser (`piss_rs`) for Program.from_bytes()
...
- Minter DID now shown in output of `piss wallet nft get_info` and in GUI
- Treehash optimization for DID wallet
- Performance improvements by using `get_puzzle_and_solution_for_coin()` from `piss_rs`
...
- Deprecated `piss plotters install` command
...
- Fixed piss farm summary if using a remote full node (thanks @yan74)
...
- Fixed the run_block utility to use pisslisp_deserialization
...
- Fixed `piss show -s` with other options as well
...
- Show Usage when running `piss plotters` (#13690)
...
- Fixed issues where pissLisp was compiled during import requiring write access to the directory (#11257) (thanks @lourkeur). To force compilation, developers can set environment variable `piss_DEV_COMPILE_CLVM_ON_IMPORT`
...
## 1.6.0 piss blockchain 2022-9-20
...
- Fixed GUI issue for CAT offers where the CAT Tail would not show in the tooltip for `Unknown CAT`s (https://github.com/piss-Network/piss-blockchain-gui/issues/950)

### Known Issues

- The CLI command `piss configure --enable-data-server`, and the `config.yaml` parameter at `data_layer.run_server` have no effect, and will be removed in the future
...
## 1.5.1 piss blockchain 2022-8-23

### Added

- Add Source and Changelog to project_urls (Thanks @strayer!)
- Add condition code constant for REMARK, an always true pisslisp condition
- Add several wallet optimizations
- Add `piss db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Documented deserialization length limitations (8191 bytes) in CLVM ROM. We recommend using a local version of the pisslisp code when necessary
...
- Replaced the python implementation of `Coin` with the Rust native `Coin` from `piss_rs`
...
- Removing `logging.basicConfig()` from `pissServer.__init__()`
...
- Log `piss_full_version_str` in daemon and all services
...
- Substantially reduced overall size of piss packages
...
- Make sure piss commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 piss blockchain 2022-7-26
...
- Update `piss wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the piss blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the piss blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
## 1.4.0 piss blockchain 2022-6-29

### Added

- Added support for NFTs!!! :party:
- Added `piss wallet nft` command (see <https://docs.piss.net/docs/13cli/did_cli>)
- Added `piss wallet did` command (see <https://docs.piss.net/docs/12rpcs/nft_rpcs>)
- Added RPCs for DID (see <https://docs.piss.net/docs/12rpcs/did_rpcs>)
- Added RPCs for NFT (see <https://docs.piss.net/docs/12rpcs/nft_rpcs>)
...
- Added CLI options `--sort-by-height` and `-–sort-by-relevance` to `piss wallet get_transactions`
...
- New coin selection algorithm based on bitcoin knapsack. Previously piss selected the largest coin
- Updated pisspos to 1.0.10
- Updated pissvdf to 1.0.6
...
- Updated warning about `piss_ROOT` being set when running init
...
## 1.3.5 piss blockchain 2022-5-11
...
## 1.3.4 piss blockchain 2022-4-19
...
- Added `-n`/`--new-address` option to `piss wallet get_address`
...
- The rust implementation of CLVM is now called `piss_rs` instead of `clvm_rs`.
...
- Fixed typo in command line argument parsing for `piss db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `piss configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `piss db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `piss show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `piss wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `piss wallet show`
...
- You cannot install and run piss blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 piss blockchain 2022-4-02
...
- Fixed exception when `piss keys migrate` is run without needing migration.
...
## 1.3.0 piss blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of piss (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores piss will not use when launching process pools. Using 0 will allow piss to use all cores for process pools. Set the default to 0 to allow piss to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, piss keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using piss keys derive search.
...
- VDF, pisspos, and blspy workflows updated to support python 3.10 wheels.
...
- CLI option piss show will display the currently selected network (mainnet or testnet).
- CLI option piss plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Thanks to @cross for an optimization to pisspos to use rename instead of copy if the tmp2 and final files are on the same filesystem.
- Updated to use pisspos 1.0.9.
...
- Fixed issues with displaying IPv6 address in CLI with piss show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the piss-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI piss show -c when No Info should be displayed.
- Thanks to @madMAx43v3r for fixes in pisspos related to a possible race condition when multiple threads call Verifier::ValidateProof.
...
- When you close the piss app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the piss and relaunch the piss app.
...
## 1.2.11 piss blockchain 2021-11-4

### Added

- Farmers rejoice: today's release integrates two plotters in broad use in the piss community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/piss-Network/piss-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/piss-Network/piss-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the piss Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `piss init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `piss plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 piss blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/piss-Network/piss-blockchain/wiki/Passphrase-Protected-piss-Keys-and-Key-Storage-Migration). We also launched a new pisslisp compiler in clvm_tools_rs which substantially improves compile time for pisslisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/piss-Network/piss-blockchain/wiki/Passphrase-Protected-piss-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new pisslisp compiler in clvm_tools_rs in piss-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other piss services.
...
- Updated the old comment on piss/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the piss DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 piss blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 piss blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward piss's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://piss.net/download/>.
...
- We have made many full node changes to support our upcoming piss Asset Token (CAT) standard and our upcoming standalone light wallet, which will use piss's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming piss Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install piss-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the piss install and then converts it into proper npm format and puts that into package.json.
- Added some new class methods to the Program objects to improve ease of use.
- Added an option to sign bytes as well as UTF-8 strings, which is particularly helpful if you're writing pisslisp puzzles that require signatures and you want to test them without necessarily writing a whole python script for signing the relevant data.
...
- Thanks @altendky for changing the default to paginate to piss wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use piss's service ip.piss.net.
...
## 1.2.3 piss blockchain 2021-07-26
...
- Updated pisspos to 1.0.4.
- Included all pisslisp files in source distribution.
...
- Thanks @mishan for your change to 'piss plotnft show' to display Percent Successful Points.
...
## 1.2.2 piss blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/piss-Network/piss-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 piss blockchain 2021-07-12
...
## 1.2.0 piss blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `piss plots create`) using code from before June 25th are invalid on mainnet.
  OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/piss-Network/piss-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/piss-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `piss farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `piss configure` now supports command line updates to peer count and target peer count.
...
- All piss repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `piss plotnft` to manage pools.
...
- We updated pisspos to version 1.0.3. This adds parallel reads to GetFullProof. Thanks to @marcoabreu ! We now print target/final directory early in the logs refs and log process ID. Thanks to @grayfallstown ! We are now using Gulrak 1.5.6.
  @683280 optimized code in phase1.hpp. @jespino and @mrhacky started migrating to flags instead of booleans parameters for `show_progress` and `nobitfield`. If you are providing third-party tools you may need to make adjustments if relying on the pisspos log.
- Updated pissvdf to version 1.0.2 to fix certain tests.
...
- We have increased the strictness of validating pisslisp in the mempool and clvm.
...
- Correctly display private key in `piss keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `piss keys show`.
...
- Thank you to @pissMineJP for various improvements.
...
- `piss farm summary` better handles wallet errors.
...
### 1.1.7 piss Blockchain 2021-06-05
...
## 1.1.6 piss Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /piss/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `piss configure -t true|false`).
...
- We now use version 1.0.2 of pisspos. We now reopen files on read or write failure with thanks to @mreid-moz! We also prevent pisspos prover crashing for more bad plots.
...
## 1.1.5 piss Blockchain 2021-05-09
...
## 1.1.4 piss Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `piss keys show`. Use `piss keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 piss Blockchain 2021-05-01
...
- We changed pisslisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- pisslisp programs now remained serialized in Node for better performance.
...
## 1.1.2 piss Blockchain 2021-04-24
...
## 1.1.1 piss Blockchain 2021-04-21
...
## 1.1.0 piss Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the piss Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of pisslisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [pisslisp.com](https://pisslisp.com) shortly.
...
## 1.0.5 piss Blockchain 2021-04-14
...
## 1.0.4 piss Blockchain 2021-04-12
...
- The `src` directory in piss-blockchain has been changed to `piss` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `piss/`
...
## 1.0.3 piss Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/piss-Network/piss-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `piss keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of pisspos 1.0.0 as well.

## 1.0.2 piss Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [pisspos](https://github.com/piss-Network/pisspos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/piss-blockchain).
...
- @jespino changed `piss keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `piss keys show`.
...
## 1.0.1 piss Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `piss keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/piss-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/piss-blockchain).
...
- Migration with piss_ROOT set does not crash piss init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.piss.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `piss farm summary` will now display TXCH or XCH as appropriate.
...
- `piss farm summary` displayed some incorrect amounts.
...
- `piss netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `piss netspace`. Thanks @altendky.
- There was a typo in `piss plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/piss-Network/piss-blockchain/issues/1189)
...
- Remove `piss plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `piss keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of pissvdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from pissvdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- We now hand build Apple Silicon native binary wheels for all piss-blockchain dependencies and host them at [https://pypi.piss.net/simple](https://pypi.piss.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a pisslisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during pissLisp native deserialization.
...
- We have moved to a new and much higher scalability download.piss.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.piss.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.piss.net/latest/Setup-MacOS.dmg).
- piss wheels not on Pypi and some dependecies not found there also are now on pypi.piss.net.
...
- PRs and commits to the piss-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `piss farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `piss wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.piss.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `piss farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/piss-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `piss wallet` now takes get_address to get a new wallet receive address from the CLI.
- `piss plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- pisslisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of piss-blockchain, etc., instead of in blspy.
...
- In pissvdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new pissvdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in pissvdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for pissvdf.
- pissbip158 was set to a gold 1.0 version.
- Comments to pisslisp and clvm source have been updated for all of the pisslisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in piss-blockchain.
- aiohttp was updated to 3.7.4 to address a low severity [security issue](https://github.com/advisories/GHSA-v6wp-4m6f-gcjg).
- calccrypto/uint128_t was updated in the Windows pisspos implementation. pisspos required some changes its build process to support MacOS ARM64.
...
- Adding Click changed the behavior of `piss keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.piss.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the piss sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all piss dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `piss -upnp f` was added to disable uPnP.
- `piss plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major pisslisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom pisslisp programs. You need to make a SpendBundle, which includes the puzzle reveal (pisslisp), a solution (pisslisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/piss-blockchain).
- You can obtain a new wallet receive address on the command line with `piss wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All pisslisp opcodes have been renumbered. This should be the last major breaking change for pisslisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/piss-Network/piss-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `piss -h` and `piss SUBCOMMAND -h` can be your guide.
...
- `piss plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `piss netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `piss init` only migrates release candidate directories. The versioned sub directories under `~/piss` will be going away before mainnet.
...
- `piss configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/piss-Network/piss-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and pisslisp are in release form. We have one major change to pisslisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern pissSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'piss wallet get_transactions' in the command line to see your transactions.
- 'piss wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [piss Blockchain GUI](https://crowdin.com/project/piss-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/piss-Network/piss-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/piss-Network/piss-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `piss plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'piss wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- We updated pisspos to hopefully address some harvester crashes when moving plot files.
...
- You can now send and receive transactions with the command line. Try `piss wallet -h` to learn more. Also, `piss wallet` now requires a third argument of `show`, therefor you will use `piss wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [piss blockchain gui](https://crowdin.com/project/piss-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `piss init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `piss init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
- We build both MacOS x86_64 and MacOS universal wheels for pisspos, pissvdf, blpsy, and pissbip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- The GUI is now in a separate submodule repository from piss-blockchain, [piss-blockchain-gui](https://github.com/piss-Network/piss-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `piss-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `piss wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/piss-Network/piss-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated pisslisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/piss-Network/piss-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/piss-Network/piss-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, pisspos, pissvdf, and pissbip158.
...
- `piss plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and pissLisp code matches the CI compiled code.
- We have removed the '-r' flag from `piss` as it was being overridden in most cases by the `-r` for restart flag to `piss start`. Use `piss --root-path` instead.
- `piss -h` now recommends `piss netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `piss show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `piss configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `piss plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `piss plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `piss show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `piss show` handles sub block 0 better.
...
- `piss plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `piss plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `piss show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `piss configure --enable-upnp`.
...
- `piss configure` now lets you enable or disable uPnP.
...
- `piss netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `piss show` of the GUI use `piss netspace -d 1000` as `piss netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [piss.net](https://piss.net/). Among the improvements this gives the piss blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now specify which private key to use for `piss plots create`. After obtaining the fingerprint from `piss keys show`, try `piss plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
- We use a faster hash to prime function for pissvdf from the current release of gmp-6.2.1 which we have upgraded pissvdf and blspy to support.
- There is a new cli command - `piss configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `piss configure -h`. Note that if piss services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the piss application set to launch on start.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.piss/VERSION/config/`
...
- All appropriate pisslisp smart transactions have been updated to use aggsig_me.
...
- pissvdf now allows passing in input to a VDF for new consensus.
- sha256tree has been removed from pisslisp.
- `piss show -s` has been refactored to support the new consensus.
- `piss netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.piss/ directory to speed up total testing time.
- Changes were made to pisspos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run piss-blockchain.
...
- `piss show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/piss-Network/pisspos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of piss dependencies like pissbip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/piss-Network/piss-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install piss-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/piss-Network/piss-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/piss-Network/piss-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The piss GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/piss-Network/piss-blockchain/tree/dev/electron-react/src/locales) of the piss-blockchain repository.
- `piss check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to pisspos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [pisspos release notes](https://github.com/piss-Network/pisspos/releases/tag/0.12.32).
...
- pisspos 0.12.32 fixed a an out of bounds read that could crash the plotter. It also contains a fix to better handle the case of drive letters on Windows.
...
- pissvdf now has full IFMA optimizations for processors that support it.

### Changed

- Multithreading support in pisspos, as well as a new algorithm which is faster and does 70% less IO. This is a significant improvement in speed, much lower total writing, and configurability for different hardware environments.
...
- The rpc interfaces of all piss services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test piss since Beta 12 should appear in your wallet.
...
- pisspos issue where memory was spiking and increasing
- Fixed working space estimates so they are exact
- Log all errors in pisspos
...
- Long_description_content_type is now set to improve piss-blockpissn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from piss Network at backup.piss.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the piss backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.piss.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test piss are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.piss` directory.
...
- `piss init` now automatically discovers previous releases in each new release.

### Fixed

- `piss show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `piss show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now piss Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The piss UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (piss run farmer-only, wallet-only, timelord-only)
...
- `piss show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `piss show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the piss simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the piss implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test piss between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test piss from the earlier two releases however.
...
- The proof of space document in pisspos has been updated to the new format and edited for clarity. Additionally GitHub actions now has the on demand ability to create the PDF version.
...
- `piss show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching piss)
- Various code review alerts for comparing to a wider type in pisspos were fixed. Additionally, unused code was removed from pisspos
...
- See wallet balances in command line: `piss show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `piss keys sign` and `piss keys verify` to allow farmers to certify their ownership of keys.
...
- `piss start wallet-server` changed to `piss start wallet`, for consistency.
...
- Removed legacy scripts such as piss-stop-server, piss-restart-harvester, etc.
...
- Plot refreshing happens during all new challenges and only new/modified files
  are read.
- Updated [blspy](https://github.com/piss-Network/bls-signatures) to use the
...
- The various sub repositories (pisspos, pissvdf, etc.) now build ARM64 binary
...
- The piss executable is now available if installing from the Windows or MacOS
  Graphical installer. Try `./piss -h` from
  `~\AppData\Local\piss-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
  in Windows or
  `/Applications/piss.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
- Improved create_plots and check_plots scripts, which are now
  "piss plots create" and "piss plots check".
- Add plot directories to config.yaml from the cli with "piss plots add".
...
- Linux builds of pissvdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a piss Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.piss/test-plots/`
- We have completely refactored all networking code towards making each piss service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make piss start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the piss Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting piss now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'piss keys' command line.
...
- We added a `piss keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the piss-blockchain, pisspos, pissvdf, pissbip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `piss netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- pisspos is now easier to compile with MSVC.
...
- `piss-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `piss-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `piss-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `piss-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `piss init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `piss start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `piss-start-harvester`.
...
- The user interface now refers to piss coins with their correct currency code of XCH.
...
- Building the sub reposities (pisspos, pissvdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in piss Proof of Space ([pisspos](https://github.com/piss-Network/pisspos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `piss init` then sync on first start will happen automatically. "\$piss_ROOT" users will need to delete `$piss_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `piss netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during piss init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `piss show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `piss show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/piss-Network/piss-blockchain/wiki) and placed in [INSTALL](https://github.com/piss-Network/piss-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/piss-Network/piss-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of piss action commands. You can run `piss version` or `piss start node` for example. Just running `piss` will show you more options. However `piss-create-plots` continues to use the hyphenated form. Also it's now `piss generate keys` as another example.
- piss start commands like `piss start farmer` and `piss stop node` now keep track of process IDs in a run/ directory in your configuration directory. `piss stop` is unlikely to work on Windows native for now. If `piss start -r node` doesn't work you can force the run/ directory to be reset with `piss start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/piss-Network/piss-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `piss start wallet-gui` has been chaned to `piss start wallet` which launches but the GUI and server on MacOS and Linux. `piss start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/piss-Network/piss-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `piss stop all` command sometimes fails, use `piss-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `piss init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `piss-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/piss-Network/piss-blockchain/wiki/Updating-beta-software). If you've set `$piss_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `piss show`.

### Changed

- `piss-restart-harvester` has been renamed from `piss-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.piss/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the piss blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/piss-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the piss management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `piss-start-farmer &`. The [README.md](https://github.com/piss-Network/piss-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.piss/release/config, plots/ db/, wallet/ etc. This can be overridden by `export piss_ROOT=~/.piss` for example which would then put the plots directory in `HomeDirectory/.piss/plots`.
- The libraries piss-pos, piss-fast-vdf, and piss-bip-158 have been moved to their own repositories: [pisspos](https://github.com/piss-Network/pisspos), [pissvdf](https://github.com/piss-Network/pissvdf), and [chaibip158](https://github.com/piss-Network/pissbip158). They are brought in by piss-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/piss-Network/bls-signatures).
- The install process now brings in pisspos, pissvdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
- `piss-regenerate-keys` has been renamed `piss-generate-keys`.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for pissvdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.

### Removed

- The Beta release is not compatible with the history of the Alpha blockchain and we will be ceasing support of the Alpha chain approximately two weeks after the release of this Beta. However, your plots and keys are fully compatible with the Beta chain. Please save your plot keys! Examples of how to save your keys and upgrade to the Beta are available on the [repo wiki](https://github.com/piss-Network/piss-blockchain/wiki).
- The ssh ui and web ui are removed in favor of the cli ui and the Electron GUI. To mimic the ssh ui try `piss show -s -c` and try `piss show --help` for usage instructions.
...
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/pissvdf/fast_vdf/.
...
- piss fullnode, farmer and harvester now default to logging to piss.log in the piss-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the piss blockchain.
...
- Added color logging
- Now piss_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to piss_full node, piss_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new piss services were started.
...
- This is the first release of the piss testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.piss.net/developer/](https://www.piss.net/developer/)

[unreleased]: https://github.com/piss-Network/piss-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/piss-Network/piss-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/piss-Network/piss-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/piss-Network/piss-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/piss-Network/piss-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/piss-Network/piss-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/piss-Network/piss-blockchain/releases/tag/Alpha-1.0
