...
"""
run_block: Convert an encoded FullBlock from the piss blockchain into a list of transactions

As input, takes a file containing a [FullBlock](../piss/types/full_block.py) in json format
...
$ca_root is the directory containing your current piss config files
$hash is the header_hash of the [BlockRecord](../piss/consensus/block_record.py)
...
The information for each spend is an "NPC" (Name, Puzzle, Condition):
        ...
        "conditions": a list of condition expressions, as in [condition_opcodes.py](../piss/types/condition_opcodes.py)
...
import click
from piss_rs import ConsensusConstants

from piss._tests.util.run_block import run_json_block
from piss.consensus.constants import replace_str_to_bytes
from piss.consensus.default_constants import DEFAULT_CONSTANTS
from piss.util.config import load_config
from piss.util.default_root import DEFAULT_ROOT_PATH
