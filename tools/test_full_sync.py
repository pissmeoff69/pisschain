...
import click
import zstd
from piss_rs import FullBlock

from piss._tests.util.full_sync import FakePeer, FakeServer, run_sync_test
from piss.cmds.init_funcs import piss_init
...
from piss.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from piss.full_node.full_node import FullNode
from piss.server.ws_connection import WSpissConnection
...
async def run_sync_checkpoint(
    ...
    piss_init(root_path, should_check_keys=False, v1_db=False)
    ...
    async with full_node.manage():
        peer: WSpissConnection = FakePeer()  # type: ignore[assignment]
