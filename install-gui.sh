...
if [ "$(id -u)" = 0 ]; then
  echo "The piss Blockchain GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in piss-blockchain-gui
SUBMODULE_BRANCH=$1
...
# https://github.com/piss-Network/piss-blockchain/pull/10460#issuecomment-1054492495
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `piss version` to version property of GUI's package.json
...
echo ""
echo "piss blockchain install-gui.sh completed."
echo ""
