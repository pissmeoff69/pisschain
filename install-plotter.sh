...
get_madmax_filename() {
  ...
  export piss_PLOT="piss_plot"
  if [ "$KSIZE" = "k34" ]; then
    export piss_PLOT="piss_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
  ...
  echo "${piss_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
