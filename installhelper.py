...
# Gets the version from the result of "piss version"
...
def get_piss_version() -> str:
    version: str = "0.0"
    piss_executable = shutil.which("piss")
    if piss_executable is None:
        piss_executable = "piss"
    output = subprocess.run([piss_executable, "version"], capture_output=True, check=False)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/piss-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/piss-blockchain-gui/packages/gui/package.json")
